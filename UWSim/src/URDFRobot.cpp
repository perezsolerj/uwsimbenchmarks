...
URDFRobot::URDFRobot(osgOcean::OceanScene *oscene,Vehicle vehicle): KinematicChain(vehicle.nlinks, vehicle.njoints) {
   ...
   for(int i=0;i<vehicle.nlinks;i++){
	if(vehicle.links[i].geom->type==0){
	  //Find file in the package
  	  if(vehicle.links[i].geom->file.rfind("/robot/"))
    	    vehicle.links[i].geom->file.erase(0,vehicle.links[i].geom->file.rfind("/robot/")+7);
	}
   }
   ...
   for(int i=0; i<vehicle.nlinks;i++) {
	...
	if(vehicle.links[i].geom->type==0){
	  link[i] = osgDB::readNodeFile(vehicle.links[i].geom->file);
	  if(link[i] == NULL){
	     std::cerr<<"Error reading file " << vehicle.links[i].geom->file <<" Check URDF file." <<std::endl;
	     exit(0);
	  }
        }
	else if(vehicle.links[i].geom->type==1){
	  link[i] = UWSimGeometry::createOSGBox(osg::Vec3(vehicle.links[i].geom->boxSize[0], vehicle.links[i].geom->boxSize[1], vehicle.links[i].geom->boxSize[2]));
	}
	else if(vehicle.links[i].geom->type==2)
	  link[i] = UWSimGeometry::createOSGCylinder(vehicle.links[i].geom->radius,vehicle.links[i].geom->length);
	else if(vehicle.links[i].geom->type==3)
	  link[i] = UWSimGeometry::createOSGSphere(vehicle.links[i].geom->radius);
	else if(vehicle.links[i].geom->type==4)
